...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (query.length < 2 || !catalog.length) {
    ...
    try {
      const openAiResponse = await fetch(OPENAI_RESPONSES_URL, {
        ...
        body: JSON.stringify({
          ...
          text: { format: { type: "json_schema", name: "topic_suggestion", strict: true, schema: RESPONSE_SCHEMA } },
          input: [
            { role: "developer", content: [{ type: "input_text", text: buildInstruction(catalog) }] },
      ...
      const suggestion = normalizeSuggestion(parsed, catalog);
      if (!suggestion) {
